...
class RankingSource(Protocol):
    ...
    def fetch(
        ...
        """Fetch and normalize a contest payload for one platform."""


class PintiaRankingSource:
    source = "pintia"
    ...
    def fetch(
        ...
        return normalize_pintia_rankings(data, fetched_at)


class NowcoderRankingSource:
    source = "nowcoder"
    ...
    def fetch(
        ...
        return normalize_nowcoder_rankings(pages, fetched_at, competition_id)


class XcpcioRankingSource:
    source = "xcpcio"
    ...
    def fetch(
        ...
        return xcpcio.fetch_contest_payload(
            competition_id,
            fetched_at,
            data_url=data_url,
...
class RankingSourceRegistry:
    sources: dict[str, RankingSource]

    @classmethod
    def defaults(cls) -> "RankingSourceRegistry":
        sources: list[RankingSource] = [
            PintiaRankingSource(),
            NowcoderRankingSource(),
            XcpcioRankingSource(),
        ]
        return cls({source.source: source for source in sources})
    ...
    def fetch(
        ...
        source_name = str(contest["source"])
        try:
            source = self.sources[source_name]
        except KeyError as exc:
            raise ValueError(f"不支持的榜单来源：{source_name}") from exc
        return source.fetch(contest, args, cookie, fetched_at)
...
def contest_index_payload(
    contests: list[dict[str, Any]],
    generated_at: str,
) -> dict[str, Any]:
    primary_contest = primary_contest_from(contests)
    return {
        "generated_at": generated_at,
        "default_contest_id": primary_contest["id"] if primary_contest else "",
        "contests": contest_options(contests),
    }
...
def fetch_contest_payload(
    ...
    cookie: str | None,
    fetched_at: str,
    source_registry: RankingSourceRegistry | None = None,
) -> dict[str, Any]:
    ...
    registry = source_registry or RankingSourceRegistry.defaults()
    payload = registry.fetch(contest, args, cookie, fetched_at)
...
def contest_label(contest: dict[str, Any]) -> str:
    return str(
        contest.get("display_name")
        or contest.get("name")
        or f'{contest.get("source")} {contest.get("competition_id")}'
    )
...
class ContestDiscoveryService:
    ...
    def discover(
        ...
        last_discovery_at["nowcoder"] = self.discover_nowcoder(
            last_discovery_at.get("nowcoder"),
            force=force,
        )
        last_discovery_at["xcpcio"] = self.discover_xcpcio(
...
def discover_nowcoder_contests(
    ...
    return ContestDiscoveryService(args).discover_nowcoder(last_discovery_at, force=force)
...
def discover_xcpcio_contests(
    ...
    return ContestDiscoveryService(args).discover_xcpcio(last_discovery_at, force=force)
...
def discover_contests(
    ...
    return ContestDiscoveryService(args).discover(last_discovery_at, force=force)
...
class RankingUpdateService:
    args: argparse.Namespace
    cookie: str | None
    source_registry: RankingSourceRegistry
    discovery_service: ContestDiscoveryService
    ...
    def create(
        ...
        return cls(
            args=args,
            cookie=cookie,
            source_registry=RankingSourceRegistry.defaults(),
            discovery_service=ContestDiscoveryService(args),
        )

    def fetch_contest_payload(self, contest: dict[str, Any], fetched_at: str) -> dict[str, Any]:
        return fetch_contest_payload(
            contest,
            self.args,
            self.cookie,
            fetched_at,
    ...
    def update_once(
        ...
        for contest in contests:
            if due_contest_ids is not None and contest["id"] not in due_contest_ids:
                continue
            try:
                payload = self.fetch_contest_payload(contest, fetched_at)
            except Exception as exc:
                if not skip_failed_contests:
                    raise
                print(
                    f"[{datetime.now().astimezone().isoformat(timespec='seconds')}] "
                    f"比赛更新失败，已跳过：id={contest['id']} "
                    f"name={contest_label(contest)} error={exc}",
                    file=sys.stderr,
                    flush=True,
                )
                continue
    ...
    def due_contest_ids(self, next_due_at: dict[str, float]) -> set[str]:
        contests = load_contest_configs(self.args)
    ...
    def next_loop_wait_seconds(
        ...
        discovery_wait = self.next_discovery_wait_seconds(last_discovery_at)
        if discovery_wait is not None:
            waits.append(discovery_wait)
        return min(waits) if waits else resolve_running_interval(self.args)
    ...
    def run_update_loop(
        ...
        while True:
            try:
                discovery_state = self.discovery_service.discover(discovery_state)
                self.publish_contest_index()
                due_ids = self.due_contest_ids(next_due_at)
                if due_ids:
                    latest_csv, row_count, intervals = self.update_once(due_ids)
                    now_monotonic = time.monotonic()
                    for contest_id in due_ids:
                        interval = intervals.get(contest_id)
                        if interval is None:
                            interval = min(resolve_running_interval(args), 60)
            ...
            interval = self.next_loop_wait_seconds(next_due_at, discovery_state)
...
def update_once(
    ...
    return RankingUpdateService.create(args, cookie).update_once(due_contest_ids)


def due_contest_ids(args: argparse.Namespace, next_due_at: dict[str, float]) -> set[str]:
    return RankingUpdateService.create(args, None).due_contest_ids(next_due_at)
...
def next_discovery_wait_seconds(
    ...
    return RankingUpdateService.create(args, None).next_discovery_wait_seconds(last_discovery_at)
...
def next_loop_wait_seconds(
    ...
    return RankingUpdateService.create(args, None).next_loop_wait_seconds(
        next_due_at,
        last_discovery_at,
    )
...
def run_update_loop(
    ...
    RankingUpdateService.create(args, cookie).run_update_loop(stop_event, last_discovery_at)
    ...
    def send_head(self) -> Any:
        if self.path.split("?", 1)[0].endswith(".json"):
            ...
            if "gzip" in accept_encoding.lower():
                json_path = Path(self.translate_path(self.path))
...
def serve_output(args: argparse.Namespace, cookie: str | None) -> int:
    ...
    update_service = RankingUpdateService.create(args, cookie)

    # Generate the first page before opening the server, so latest.html exists.
    last_discovery_at = update_service.discovery_service.discover(
        {"nowcoder": None, "xcpcio": None},
        force=True,
    )
    latest_csv, row_count, _ = update_service.update_once()
    print(
        f"[{datetime.now().astimezone().isoformat(timespec='seconds')}] "
        f"已更新 {row_count} 支队伍 -> {latest_csv}",
        flush=True,
    ...
    worker = threading.Thread(
        target=update_service.run_update_loop,
        args=(stop_event, last_discovery_at),
        daemon=True,
    )
    worker.start()
...
def main() -> int:
    ...
    try:
        RankingUpdateService.create(args, cookie).run_update_loop()
    except Exception:
        return 1
    return 0
